...
function openContractModal(existingId = null) {
  ...
  if (existingId) {
    ...
    if (c) {
      ...
      (function(){var _e=document.getElementById('sigTypeInput');if(_e)_e.value=c.signature || '';})()
    }
  } else {
    ...
    (function(){var _e=document.getElementById('sigTypeInput');if(_e)_e.value='';})()
...
function bindTimelineModal() {
  const closeTl = () => el.tlModal.classList.add('hidden');
  const tlCloseBtn = document.getElementById('tlModalClose');
  if (tlCloseBtn) tlCloseBtn.addEventListener('click', closeTl);
  el.tlModalCancel.addEventListener('click', closeTl);
