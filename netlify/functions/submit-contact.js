...
const TBL_INQUIRIES  = 'tblwfk7tmoFhGIia5';
const TBL_NEWSLETTER = 'tblSMb3y7vrvjbONx';
const LIST_INQUIRIES  = 5;
const LIST_NEWSLETTER = 2;
const TPL_INQUIRIES   = 5;
...
async function sendBrevoEmail(templateId, email, name, params) {
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    headers: { 'api-key': BREVO_KEY, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      sender: { email: 'clients@bleuskm.com', name: 'BLEUSKM Studios' },
...
exports.handler = async function(event) {
  ...
  try {
    ...
    const { name, email, phone, projectType, budget, concept, timeline, newsletter } = body;
    ...
    await saveToAirtable(TBL_INQUIRIES, fields);
    await addBrevoContact(LIST_INQUIRIES, email, name, { PROJECT_TYPE: projectType || '' });
    await sendBrevoEmail(TPL_INQUIRIES, email, name, { PROJECT_TYPE: projectType || '' });
    if (newsletter) {
