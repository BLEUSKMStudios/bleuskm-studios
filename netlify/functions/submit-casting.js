...
const TBL_CASTING    = 'tblLGmXULNb9ebFxH';
const TBL_NEWSLETTER = 'tblSMb3y7vrvjbONx';
const LIST_CASTING    = 4;
...
exports.handler = async function(event) {
  ...
  try {
    ...
    const { name, email, phone, age, location, role, reel, headshot, about, film, newsletter } = body;
    if (!email || !name || !film) return respond(400, { error: 'Missing required fields' });
    const fields = { 'Name': name, 'Email': email, 'Film': film, 'Role': role || '', 'Casting Status': 'Reviewing' };
    if (phone) fields['Phone'] = phone;
    if (age) fields['Age'] = parseFloat(age);
    if (location) fields['Location'] = location;
    if (reel) fields['Reel/Portfolio Link'] = reel;
    if (headshot) fields['Headshot'] = headshot;
    if (about) fields['About'] = about;
    await saveToAirtable(TBL_CASTING, fields);
    await addBrevoContact(LIST_CASTING, email, name, { FILM: film, ROLE: role || '' });
    await sendBrevoEmail(TPL_CASTING, email, name, { FILM: film, ROLE: role || '' });
    if (newsletter) {
    ...
    console.error('submit-casting error:', err);
