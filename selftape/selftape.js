...
function getRoleKey(role) {
  ...
  if (r.includes('patron') || r.includes('table patron')) return 'patron';
  return 'unknown';
}
...
const ROLE_CONTENT = {
  ...
  player: {
    ...
    instructions: `
      <div class="instr-block">
        ...
        <p class="instr-text" style="margin-top:12px;">You may paraphrase slightly if it feels natural.</p>
      </div>
      <div class="instr-block">
  ...
  bartender: {
    ...
    instructions: `
      <div class="instr-block">
        <p class="instr-label">DIRECTION</p>
        <p class="instr-text">Submit a 30 to 60 second tape.</p>
        <p class="instr-text" style="margin-top:8px;">Perform simple actions:</p>
        <ul class="instr-list" style="margin-top:8px;">
    ...
    scenes: [
      {
        label: 'LINE',
        lines: [
          { char: 'BARTENDER', text: "You want anything?", reader: false },
        ],
  ...
  patron: {
    ...
    instructions: `
      <div class="instr-block">
        <p class="instr-label">DIRECTION</p>
        <p class="instr-text">Submit a 30 to 60 second behavioral tape.</p>
        <p class="instr-text" style="margin-top:8px;">You may:</p>
        <ul class="instr-list" style="margin-top:8px;">
      ...
      <div class="instr-block">
        ...
        <p class="instr-text">At some point during the tape, pause briefly as if something in the room shifted. Then continue naturally. No dialogue required.</p>
      </div>
      <div class="instr-block">
        <p class="instr-label">FOCUS</p>
        <ul class="instr-list">
          <li>Natural behavior</li>
          <li>Subtle environmental awareness</li>
          <li>Presence without dialogue</li>
        </ul>
      </div>`,
    scenes: []
  },
  ...
  couple: {
    ...
    instructions: `
      <div class="instr-block">
        <p class="instr-label">DIRECTION</p>
        <p class="instr-text">Submit a 30 to 60 second scene, together or solo.</p>
        <p class="instr-text" style="margin-top:8px;">Create:</p>
        <ul class="instr-list" style="margin-top:8px;">
...
function renderRole() {
  ...
  if (content.scenes && content.scenes.length > 0) {
    content.scenes.forEach(scene => {
      html += `<div class="scene-block" style="margin-top:24px;">`;
      html += `<span class="scene-label">${scene.label}</span>`;
      ...
      if (scene.custom) {
        ...
        scene.lines.forEach(line => {
          if (line.direction) {
            html += `<div class="line-direction">${esc(line.text)}</div>`;
          } else {
            html += `
              <div class="line">
                <span class="line-char ${line.reader ? 'reader' : ''}">${esc(line.char)}</span>
                <span class="line-text ${line.reader ? 'reader-text' : ''}">${esc(line.text)}</span>
              </div>`;
          }
        });
        html += `</div>`;
      ...
      if (scene.note) {
        html += `<p class="scene-note">${esc(scene.note)}</p>`;
      }
...
submitBtn.addEventListener('click', async () => {
  ...
  if (!link) {
    showError('Please paste your self-tape link before submitting.');
    return;
  }
  ...
  if (!recordId) {
    showError('Missing record ID. Please use the link from your callback email.');
    return;
  }
